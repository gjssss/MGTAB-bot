...
_load = partial(torch.load, weights_only=False)

class Cresci15(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None):
        super().__init__(root, transform, pre_transform)
        self.data, self.slices = _load(self.processed_paths[0])
        self.root = root
    ...
    def process(self):
        ...
        edge_index = _load(self.root + "/edge_index.pt")
        edge_type = _load(self.root + "/edge_type.pt")
        label = _load(self.root + "/label.pt")
        cat_prop = _load(self.root + "/cat_properties_tensor.pt")
        num_prop = _load(self.root + "/num_properties_tensor.pt")
        des_tensor = _load(self.root + "/des_tensor.pt")
        tweets_tensor = _load(self.root + "/tweets_tensor.pt")
        ...
        train_idx = _load(self.root + "/train_idx.pt")
        val_idx = _load(self.root + "/test_idx.pt")
        test_idx = _load(self.root + "/val_idx.pt")
...
class MGTAB(InMemoryDataset):
    ...
    def process(self):
        ...
        edge_type = _load(self.root + "/edge_type.pt")
        edge_weight = _load(self.root + "/edge_weight.pt")
        stance_label = _load(self.root + "/labels_stance.pt")
        bot_label = _load(self.root + "/labels_bot.pt")

        features = _load(self.root + "/features.pt")
...
class MGTABlarge(InMemoryDataset):
    ...
    def process(self):
        ...
        edge_index0 = _load(self.root + "/large_edge_index0.pt")
        edge_index1 = _load(self.root + "/large_edge_index1.pt")
        edge_index2 = _load(self.root + "/large_edge_index2.pt")
        edge_index3 = _load(self.root + "/large_edge_index3.pt")
        edge_index4 = _load(self.root + "/large_edge_index4.pt")
        edge_index5 = _load(self.root + "/large_edge_index5.pt")
        edge_index6 = _load(self.root + "/large_edge_index6.pt")
        ...
        stance_label = _load(self.root + "/labels_stance.pt")
        bot_label = _load(self.root + "/labels_bot.pt")
        features = _load(self.root + "/large_features.pt")
